...
pub fn select_notes_for_amount(
    ...
    let mut available: Vec<_> = notes.iter()
        .filter(|n| !n.is_spent)
        .cloned()
        .collect();
...
pub fn parse_merkle_path(
    witness_hex: &str,
    position: u64,
) -> SaplingResult<sapling::MerklePath> {
    ...
    for i in 0..SAPLING_TREE_DEPTH {
        ...
        let node_bytes: [u8; 32] = witness_bytes[start..end].try_into()
            .map_err(|_| SaplingError::InvalidWitness)?;
    ...
    sapling::MerklePath::from_parts(path_elems, pos)
        .map_err(|_| SaplingError::InvalidWitness)
}
...
pub fn note_from_parts(
    ...
    let address = PaymentAddress::from_bytes(&addr_bytes)
