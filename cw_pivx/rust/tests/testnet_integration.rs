...
fn test_transaction_amount_boundaries() {
    ...
    println!(
        "  Dust threshold: {} satoshis ({} PIV)",
        dust_threshold,
        dust_threshold as f64 / 1e8
    );
    println!(
        "  Max reasonable: {} satoshis ({} PIV)",
        max_reasonable,
        max_reasonable as f64 / 1e8
    );
}
...
fn test_expiry_height_handling() {
    ...
    assert_eq!(
        serialized,
        [0, 0, 0, 0],
        "Expiry height serializes to 4 zero bytes"
    );
...
fn test_consensus_constants() {
    ...
    assert_eq!(
        MAINNET_ACTIVATION, 2_700_500,
        "Mainnet Sapling at block 2,700,500"
    );
    ...
    assert_eq!(
        TX_VERSION, 0x80000003,
        "Sapling version 3 with overwintered bit"
    );
